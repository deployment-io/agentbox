...
func TestAutonomousConfigIsSingleLineJSON(t *testing.T) {
	raw, err := json.Marshal(autonomousConfig("/tmp/s.sock"))
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	if strings.Contains(string(raw), "\n") {
		t.Error("config contains newlines; opencode reads it as a file but the " +
			"single-line form is what the previous const wrote and what logs expect")
	}
}
